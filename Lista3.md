...
```mermaid
flowchart TD
A([INICIO]) --> B{{Receber número}}
B --> J[\Num\]
J --> C{Num >= 0}
C -- TRUE --> D(Par ou Ímpar)
C -- FALSE --> E{{ Número inválido}}
D -- Par --> F{{ Número é Par}}
D -- Ímpar --> G{{ Número é Ímpar}}
E --> H([FIM])
F-->H
G -->H
```
...
| Número | par | impar | saida| 
|      --      |      --      |      --      |      --      |
| 30    | V       | F    |   "O número é par"|
| 7  | F         |V       |  " O número é impar"|
| -12 | F         |F       | "Número inválido, digite um número positivo."|
